...
public class Lesson1 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Life hero in Blue side: " + darkSide.size());
        System.out.println("Life hero in Green side: " + whiteSide.size());
        in.nextLine();
    ...
    private static void isLifeInTeam(){
        int cntDie = 0;
        for (BaseHero hero: darkSide) {
            if (hero.getStatus().equals("Die")) cntDie++;
        }
        int life = darkSide.size() - cntDie;
        System.out.println("Life hero in Blue side: " + life);

        cntDie = 0;
        for (BaseHero hero: whiteSide) {
            if (hero.getStatus().equals("Die")) cntDie++;
        }
        life = darkSide.size() - cntDie;
        System.out.println("Life hero in Green side: " + life);
    }
    ...
    public static void init(){
        ...
        int max = MAX_BAND;
        for (int i = 0; i < 10; i++) {
            int tmp  = max>0 ? new Random().nextInt(5)+1 : 1;
            switch (rnd.nextInt(4)){
                case 0:
                    darkSide.add(new Peasant(darkSide, x++, y, tmp));
                    break;
                case 1:
                    darkSide.add(new Magician(darkSide, x++, y, tmp));
                    break;
                case 2:
                    darkSide.add(new Sniper(darkSide, x++, y, tmp));
                    break;
                default:
                    darkSide.add(new Robber(darkSide, x++, y, tmp));
            }
            max -= tmp;
        }
        ...
        for (int i = 0; i < 10; i++) {
            ...
            switch (rnd.nextInt(4)){
                case 0:
                    whiteSide.add(new Peasant(whiteSide, x++, y, tmp));
                    break;
                case 1:
                    whiteSide.add(new Monk(whiteSide, x++, y, tmp));
                    break;
                case 2:
                    whiteSide.add(new Crossbowman(whiteSide, x++, y, tmp));
                    break;
                default:
                    whiteSide.add(new Spearman(whiteSide, x++, y, tmp));
            }
            max -= tmp;
        }

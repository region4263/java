    ...
    private static String getChar(int x, int y){
        ...
        for (int cnt = 0; cnt < darkSide.size(); cnt++) {
            if (darkSide.get(cnt).getPosition().x == x && darkSide.get(cnt).getPosition().y == y) {
                out = "|" + ( (darkSide.get(cnt).getStatus().equals("Die")?AnsiColors.ANSI_CYAN:AnsiColors.ANSI_BLUE) + darkSide.get(cnt).getName().charAt(0) + AnsiColors.ANSI_RESET);
            } else if (whiteSide.get(cnt).getPosition().x == x && whiteSide.get(cnt).getPosition().y == y) {
                out = "|" + ( (whiteSide.get(cnt).getStatus().equals("Die")?AnsiColors.ANSI_CYAN:AnsiColors.ANSI_GREEN) + whiteSide.get(cnt).getName().charAt(0) + AnsiColors.ANSI_RESET);
            }
        }
        return out;
    }
